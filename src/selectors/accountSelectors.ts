 ...
 * 形式外はそのまま返す（落とさない）
 */
export const formatMonthJP = (monthKey: string) => {
...
/**
 * 古いlocalStorageデータなどで id が欠けてても落ちないようにする。
 */
...
/**
 * Reactのkey用。idがあればそれ、なければ内容で擬似キー。
 */
 ...
 * - draftの月は常に含める
 */
export const selectMonthOptions = (accounts: Account[], draftDate: string): string[] => {
  const set = new Set<string>()
  set.add(draftDate.slice(0, 7))

  for (const a of accounts) {
    if (typeof a.date === 'string' && a.date.length >= 7) {
 ...
 * ALL表示時のサマリなどで使う。
 */
export const selectLatestMonthKey = (accounts: Account[], draftDate: string): string => {
  ...
  const arr = Array.from(months).sort((a, b) => (a < b ? 1 : -1))
...
/**
 * 表示用ソート（安定ソート風）
 * - 収支混在時：種別 → カテゴリ順（固定）で揃えると見やすい
 */
export const selectVisibleAccounts = (accountsByMode: Account[], sortKey: SortKey) => {
  const arr = [...accountsByMode]

  // 種別の並び（収入→支出）。好みで逆にしてOK。
  const TYPE_ORDER: Record<AccountType, number> = {
    INCOME: 0,
    EXPENSE: 1,
  }

  // categories.ts の並びをそのまま固定順として使う
  const expenseEntries: Array<[string, number]> = Array.from(C.EXPENSE_CATEGORIES).map((c, i) => [
    `EXPENSE:${c}`,
    i,
  ])
  const incomeEntries: Array<[string, number]> = Array.from(C.INCOME_CATEGORIES).map((c, i) => [
    `INCOME:${c}`,
    i,
  ])
...
export const selectPieData = (accountsByPeriod: Account[], type: AccountType) => {
