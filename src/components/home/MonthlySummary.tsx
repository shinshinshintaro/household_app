  ...
  return (
    <div className="card">
      ...
      <div className="subText" style={{ marginBottom: 8 }}>
        最新月（{titleMonthKey}）の収支
      </div>

      {/* ②：ここに移植（色・フォントはクラスで維持する想定） */}
      <div className="summaryRow" style={{ marginBottom: 10 }}>
        <span className="summaryItem summaryGreen">収入：<b>{yen(income)}</b></span>
        <span className="summaryItem summaryRed">支出：<b>{yen(expense)}</b></span>
        <span className={`summaryItem ${balanceClass}`}>収支：<b>{yen(balance)}</b></span>
        <span className="summaryItem summaryBlue">目標貯金：<b>{yen(targetSavings)}</b></span>
        <span className={`summaryItem ${diffClass}`}>差：<b>{yen(diff)}</b></span>
      </div>
